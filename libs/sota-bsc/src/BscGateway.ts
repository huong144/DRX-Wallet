...
const ethMaxGasPrice = EnvConfigRegistry.getCustomEnvConfig('BSC_MAX_GAS_PRICE');
...
export class BscGateway extends AccountBasedGateway {
  ...
  public async sendRawTransaction(rawTx: string, retryCount?: number): Promise<ISubmittedTransaction> {
    ...
    try {
      ...
      logger.info(`BscGateway::sendRawTransaction with hash =${infuraReceipt.transactionHash}`);
  ...
  public async getRawTransaction(txid: string): Promise<web3_types.Transaction> {
    ...
    // @ts-ignore
    const tx = await web3.getBigGasLimitTransaction(txid);
  ...
  public async getErc20TokenInfo(contractAddress: string): Promise<IErc20Token> {
    ...
    try {
      ...
      logger.error(`BscGateway::getErc20TokenInfo could not get info contract=${contractAddress} due to error:`);
      logger.error(e);
      return null;
    }
  }
