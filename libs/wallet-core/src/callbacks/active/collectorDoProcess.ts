...
import CardanoGateway from '../../../../sota-ada/src/CardanoGateway';
...
async function _collectorDoProcess(manager: EntityManager, collector: BasePlatformWorker): Promise<void> {
  ...
  try {
    ...
    } else if (currency.symbol === BlockchainPlatform.Cardano) {
      rawTx = await _constructUtxoBasedCardanoCollectTx(records, rallyWallet.address, manager);
    } else {
